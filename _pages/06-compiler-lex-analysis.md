...
各算符均是**左结合**的，并且优先级**闭包、正闭包相等，大于连接运算，大于选择运算**
...
**FA识别(接受)的语言**指的FA识别(接受)的**串的集合**，记为$L(M)$
...
2. 对相同的属于字母表$\Sigma$的串，DFA识别路径唯一而NFA不唯一
...
​	没有固定方法，转化时考虑将FA某一部分合成一个正规式，并作为一个整体接着与其他正规式合并。
...
算法：  
1. 初始划分$\Pi$：添加死状态(指可以从初始状态到达但是一旦到达就无法前往接受状态的状态，该状态所有转换均到自身)把转换函数变为**全函数**，将M的状态划分成接受子集$F$和非接受子集$S$。
