...
由于数据均匀分布，因此各桶内元素不会过多(一般使用n个桶，和元素数量相同，因此桶内期望只有一个元素)，即**先分段有序，再段内有序**，并且分段有序是线性时间完成的，段内有序虽然是平方时间完成的但是是每一小段平方而非整个输入规模的平方，因此最终使得运行时间渐近趋近于$\Theta (n)$
