...
## 算法步骤
1. 将数组分成$\lceil \frac{n}{5} \rceil$组，每组5个元素(除了最后一组可能为$n mod 5$个)  
2. 对每一组5个元素进行插入排序，然后找到中位数
3. 对总共$\lceil \frac{n}{5} \rceil$个中位数求中位数，求中位数的方法是递归调用本算法，将待求顺序统计量设置为中位数
4. 以求得的中位数的中位数作为划分元，若划分元正好为第i顺序统计量那么返回，否则使用指定划分元的$PARTITION$算法进行划分
5. 根据i顺序统计量所处位置和划分结果，递归调用本算法继续求解  
