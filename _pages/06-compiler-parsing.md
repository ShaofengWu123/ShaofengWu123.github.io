...
**Follow符号实际上是分析A空产生式时希望看到的符号,这样可以直接结束A的分析开始下一个符号的分析**
#### First集合
...
对LL(1)文法的理解：LL(1)文法第一个“L”表示从左至右扫描，第二个“L”表示最左推导，“1”表示看1位，即根据当前指针指向符号来预测推导
{:.notice--info}
...
- LL(1)文法没有左递归，否则必然存在产生式Fisrt相同的情况,不符合定义；当然存在左递归的文法也是不能进行预测分析的，原因相同。
- 其他
...
对应递归分析程序如下。以产生式$type \rightarrow array\ [simple]\ of\ type$为例，可以看到`type()`函数中第二个`else if`正是按照该产生式的顺序进行匹配，而这个`else if`的条件也正是`lookahead==array`，即$Fisrt(type)$中的一种情况。    
![递归分析](..\assets\images\com\recursive_analysis.png)

### 5.4.4.非递归的预测分析
非递归的预测分析实际上利用**栈**来实现递归相同的功能，利用栈顶符号和当前符号**预测**应该进行的**最左推导**，并将最左推导倒序入栈，继续进行，直到整个符号串读到$（结束符），然后停机。  
...
4. 如果当前符号a是非终结符，根据分析表$M[X,a]$这一项，确定使用哪个产生式，并在X出栈后，将产生式右部**倒序入栈**（目的是让最左符号处于栈顶，从而接着进行**最左推导**）  
...
- 项目对活前缀有效：如果$S^{,} \Rightarrow ^* \alpha A \omega \rightarrow \alpha \beta _1 \beta _2 \omega$，那么称**项目$A \rightarrow \beta _1 \cdot \beta _2$对活前缀$\alpha \beta _1$有效**。实际上即**该项目处于DFA中读取该活前缀后所处状态的集合内**，并且相应活前缀经过该项目期待的输入$\beta _2$后依然是活前缀。
...
理解：LR(0)分析之所以k=0，是因为其仅根据当前栈顶状态本身的信息来判断归约还是移进(虽然要根据当前符号判断是否出错，以及移进转移到什么状态，这是必须的)，这也是LR(0)分析能力弱的原因(有移进-归约冲突)
