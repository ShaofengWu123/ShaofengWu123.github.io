...
2. $r_i$：对应归约，即将栈顶符号按第i个产生式归约，将产生式右部内容全部弹出(包括状态和符号)，将产生式左部和对应转移后状态依次入栈，然后再考虑符号a和当前栈顶状态对应动作关系。实际上，这一步即**退回**到之前的某个状态，然后根据归约得到的结果再移动到某个状态，即最左归约后再考虑后续符号。
...
构建DFA的步骤与SLR(1)类似，但是所有的项目要改为LR(1)项目，
